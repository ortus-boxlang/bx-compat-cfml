...
public class CreateUUIDTest {
	...
	@DisplayName( "It returns a bastardized UUID format to match ACF and Lucee" )
	@Test
	public void testUUIDFormatIsCFMLTraditional() {
		...
		assertThat( uuid.length() ).isEqualTo( 35 );
		String[] splitUUID = uuid.split( "-" );
		assertThat( splitUUID.length ).isEqualTo( 4 );
		...
		assertThat( splitUUID[ 3 ].length() ).isEqualTo( 16 );
	}
	...
	@DisplayName( "It forces uppercase characters" )
	@Test
	public void testIsForcedUppercase() {
		instance.executeSource(
		    """
		    result = createUUID();
		    """,
		...
		String uuid = variables.getAsString( result );
		assertThat( uuid.matches( ".*[a-f].*" ) ).isEqualTo( false );
	}
