...
public final class BlowFishEasy {
	...
	public String decryptString( String sCipherText ) {
		...
		if ( nLen < this.BLOCKSIZE )

			return null;

		// get the CBC IV

		byte[]	cbciv		= new byte[ this.BLOCKSIZE ];
	...
	public static final class BinConverter {
		...
		public static long byteArrayToLong( byte[] buffer, int nStartIndex ) {
			...
			return ( ( ( long ) buffer[ nStartIndex ] ) << 56 ) | ( ( buffer[ nStartIndex + 1 ] & 0x0ffL ) << 48 )
			    | ( ( buffer[ nStartIndex + 2 ] & 0x0ffL ) << 40 )
			    | ( ( buffer[ nStartIndex + 3 ] & 0x0ffL ) << 32 ) | ( ( buffer[ nStartIndex + 4 ] & 0x0ffL ) << 24 )
	...
	public class BlowfishECB {
		...
		protected long encryptBlock( long lPlainBlock ) {
			...
			nLeft	^= ( ( m_sbox1[ nRight >>> 24 ] + m_sbox2[ ( nRight >>> 16 ) & 0x0ff ] ) ^ m_sbox3[ ( nRight >>> 8 ) & 0x0ff ] )
			    + m_sbox4[ nRight & 0x0ff ];
